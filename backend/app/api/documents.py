...
Write handlers own transactions: ``commit`` / ``rollback`` explicitly.
``get_db`` only yields the session (see ``app.database``).
"""
...
from app.services.text_document_ingest import ingest_text_file
