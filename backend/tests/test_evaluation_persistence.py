...
@pytest.mark.asyncio
async def test_persist_evaluation_rejects_llm_judge_without_generation(client: AsyncClient):
    async with async_session_maker() as session:
        ds = await tp.create_dataset(session, name="eval_llm_guard_ds", description="")
        qc = await tp.create_query_case(
            session,
            dataset_id=ds.id,
            query_text="q?",
    ...
    up = await client.post(
        f"{BASE}/documents",
        params={"chunk_size": 80},
        files=make_upload("corpus2.txt", CORPUS),
    )
    assert up.status_code == 201
    ...
    async with async_session_maker() as session:
        r = await session.get(Run, run_id)
        assert r is not None
        assert r.status == STATUS_RETRIEVAL_COMPLETED

        with pytest.raises(ValueError, match="used_llm_judge=True"):
            await persist_evaluation_and_complete_run(
                session,
                run_id=run_id,
                evaluation_latency_ms=1,
                total_latency_ms=retrieval_ms + 1,
                faithfulness=0.9,
                completeness=0.8,
                ...
                prerequisite_status=STATUS_RETRIEVAL_COMPLETED,
                commit=False,
            )
