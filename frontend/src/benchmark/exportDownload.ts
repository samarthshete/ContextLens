...
export function buildDashboardExportCsv(
  ...
  if (summary) {
    pushMetricRows(lines, 'summary_counts', [
      ...
      ['repeated_sampling_note', summary.repeated_sampling_note ?? ''],
      ...
      ['scale_unique_query_cases_with_runs', summary.scale?.unique_query_cases_with_runs],
      ['diagnosis_evidence_tier', summary.diagnosis_timing?.evidence_tier],
      ['diagnosis_sessions_count', summary.diagnosis_timing?.sessions_count],
      ['llm_reduction_evidence_tier', summary.llm_reduction?.evidence_tier],
      ['llm_reduction_workload_count', summary.llm_reduction?.workload_count],
      [
        'llm_reduction_median_pct',
        summary.llm_reduction?.median_llm_judge_reduction_pct_across_workloads,
      ],
      ...
      ['model_failures', summary.model_failures ?? 0],
      ...
      ['latency_total_p50_ms', summary.latency?.total_latency_p50_ms],
      ['end_to_end_run_latency_avg_sec', summary.latency?.end_to_end_run_latency_avg_sec],
      ['end_to_end_run_latency_p50_sec', summary.latency?.end_to_end_run_latency_p50_sec],
