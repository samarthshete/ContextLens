...
## Scope
- Single user  
- No per-user auth — optional **shared write key** (`CONTEXTLENS_WRITE_KEY`) for production-like / public demo deployments: non-GET `/api/v1` require header `X-ContextLens-Write-Key`; GET remains read-only without the key. Not a substitute for SSO or multi-tenant isolation (`docs/DEPLOYMENT.md`).  
- No billing  

### Managed Postgres connection string decision
- Hosts such as **Render** expose Postgres as `postgresql://user:pass@host:port/db`. ContextLens async SQLAlchemy uses **asyncpg**; settings normalize `postgresql://` and `postgres://` to **`postgresql+asyncpg://`** at load (`app.config.normalize_async_database_url`) so Blueprint `fromDatabase: connectionString` works without manual editing.

### Write protection (hosted demo) decision
- **Middleware** (`WriteProtectionMiddleware`): if `CONTEXTLENS_WRITE_KEY` is non-empty, block POST/PATCH/DELETE/PUT (any method except GET/HEAD/OPTIONS) under `/api/v1` without matching `X-ContextLens-Write-Key` (constant-time compare).  
- **`APP_ENV=production`**: startup **fails** if write key empty or `CORS_ORIGINS` uses `*` or is empty — prevents accidentally shipping an open-write public API.  
- **Docs / OpenAPI**: disabled at `/docs`, `/redoc`, `/openapi.json` when `APP_ENV=production`.  
- **UI**: `GET /api/v1/meta` exposes `write_protection` only; SPA stores the key in **sessionStorage** after `POST /api/v1/meta/verify-write-key` succeeds.  
- No plugins  
- No agents  
...
- **Dashboard cost — per run (`GET /runs/dashboard-summary` → `cost`):** **`avg_cost_usd`** = mean over **evaluation rows** with non-null **`cost_usd`** (unchanged). **`avg_cost_usd_per_llm_run`** = **`AVG`** of per-run **`SUM(cost_usd)`** restricted to **LLM-bucket** evaluation rows with non-null cost (heuristic rows excluded — they have no measured LLM USD in honest data); **`llm_runs_with_measured_cost`** = distinct **`run_id`** in that set; both **`null`** / **0** when no such runs. **`avg_cost_usd_per_full_rag_run`** / **`full_rag_runs_with_measured_cost`**: same, further restricted to runs that have a **`generation_results`** row (full RAG path). One evaluation row per **`run_id`** today, so **`SUM`** per run equals that row’s **`cost_usd`**; grouping stays correct if the schema ever allowed multiple eval rows per run.  
...
- **Dashboard retrieval latency percentiles:** **`retrieval_latency_p50_ms`** and **`retrieval_latency_p95_ms`** on **`GET /runs/dashboard-summary`** are PostgreSQL **`percentile_cont(0.5)`** / **`percentile_cont(0.95)`** with **`WITHIN GROUP (ORDER BY runs.retrieval_latency_ms)`** over rows where that column is non-null — same population as the retrieval **mean** on that endpoint. Implemented via **`app/services/phase_latency_distribution.py`**, reused by **`GET /runs/dashboard-analytics`** `latency_distribution` for all phases (no duplicate SQL). When there are **no** non-null retrieval samples, both percentiles and the mean are **`null`** (JSON omits or explicit null per schema). **Continuous** percentile semantics match existing analytics latency panels (`median_ms` / `p95_ms`).
- **End-to-end run latency (dashboard):** **`end_to_end_run_latency_avg_sec`** and **`end_to_end_run_latency_p95_sec`** are derived only from persisted **`runs.total_latency_ms`** where non-null — same population as **`avg_total_latency_ms`** on **`GET /runs/dashboard-summary`** and as **`latency_distribution.total`** avg / p95 on **`GET /runs/dashboard-analytics`**. Averages are arithmetic means in milliseconds internally, then **÷ 1000** for seconds in these named fields. P95 uses **`percentile_cont(0.95)`** with **`WITHIN GROUP (ORDER BY runs.total_latency_ms)`** (via **`phase_latency_distribution`**). **`null`** when there are no samples; **no** client-side recomputation in the UI for these aggregates.
- **Dashboard scale metrics (`GET /runs/dashboard-summary` → `scale`):** Integer counts from canonical tables only — **`benchmark_datasets`** = `COUNT(*)` on **`datasets`**; **`total_queries`** = `COUNT(*)` on **`query_cases`**; **`total_traced_runs`** = runs with **≥1** **`retrieval_results`** row **and** **≥1** **`evaluation_results`** row (same definition as **`total_traced_runs`** in **`app/metrics/aggregate.py`**); **`configs_tested`** = **`COUNT(DISTINCT runs.pipeline_config_id)`** over all run rows (same as aggregate **`configs_tested`**); **`documents_processed`** = documents with **`status = 'processed'`** (successful ingest — stricter than aggregate **`document_count`**, which counts all document rows); **`chunks_indexed`** = **`COUNT(*)`** on **`chunks`** (same as aggregate **`chunk_count`** — stored chunk rows, cascade-deleted with documents). **0** is a valid count; there is no separate “N/A” for these fields.
- **`GET /runs/config-comparison` — cross-config score spread (`ConfigScoreComparisonSummary`):** Computed in **`app/services/config_comparison.py`** from the **same** traced-run rows as the per-config table, **within one evaluator bucket** (heuristic vs LLM). **Best / worst** = **max / min** of **`avg_faithfulness`** or **`avg_completeness`** among configs with **`traced_runs > 0`** and a **non-null** average for that dimension; ties → **lowest** **`pipeline_config_id`**. **Delta %** = `100 * (best_avg - worst_avg) / worst_avg` when **`worst_avg > 1e-9`**; **`0.0`** when averages are equal; **`null`** when the denominator is not usable (e.g. worst ≈ 0). Row-level averages use SQL **`AVG(column)`** (non-null rows only). **`combine_evaluators=true`:** do **not** report faithfulness best/worst/delta (all **null**) — heuristic + LLM averages must not be blended for that claim; **completeness** spread is still computed on the merged rows.
...
- **First product slice** in `frontend/`: consumes existing `/api/v1` routes only (no new backend for this flow). **Navigation:** **client-side routing** via `react-router-dom` `BrowserRouter`; URL is the source of truth for view selection. **Routes:** `/benchmark` (run form), `/runs` (recent runs list), `/runs/:runId` (run detail deep link), `/queue` (queue browser), `/documents/:documentId` (document detail), `/compare` (config comparison), `/dashboard` (observability). `/` and unknown paths redirect to `/benchmark`. Invalid (non-numeric) `:runId` shows an inline error. Browser back/forward and refresh are supported. **Vite dev** proxies `/api` → **`BACKEND_PROXY_TARGET`** (default **`http://127.0.0.1:8002`** in `vite.config.ts`; use `frontend/.env.development.local` for e.g. Docker API on **:8000** — see `frontend/.env.example`). **`VITE_API_BASE`** at build time for cross-origin API (must match backend **`CORS_ORIGINS`**). **`WriteKeyBanner`** calls **`GET /api/v1/meta`** and prompts for the shared write key when **`write_protection`** is true; key is stored in **sessionStorage** only. **SPA fallback:** Vite dev/preview handles history API fallback by default; production static hosts (**`frontend/vercel.json`**, nginx, etc.) must route non-asset paths to `index.html` (`docs/DEPLOYMENT.md`).
